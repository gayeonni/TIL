...
class Solution {
    public int[] solution(String today, String[] terms, String[] privacies) {
        ...
        // 오늘 날짜를 기준으로 계산하기 위해 날짜를 숫자로 변환
        ...
        for (int i = 0; i < privacies.length; i++) {
            ...
            int collectedDate = convertDateToNumber(parts[0]); // 수집 날짜 변환
            String termType = parts[1]; // 약관 종류
            
            // 유효기간을 계산
            int validPeriod = termMap.get(termType);
            int expiryDate = addMonthsToDate(collectedDate, validPeriod); // 유효기간을 더한 날짜
            
            // 오늘 날짜와 비교하여 파기 여부 판단
    ...
    // 날짜를 숫자로 변환하는 메서드 (YYYY.MM.DD -> YYYYMMDD 형태)
    private int convertDateToNumber(String date) {
        String[] parts = date.split("\\.");
        int year = Integer.parseInt(parts[0]);
        int month = Integer.parseInt(parts[1]);
        int day = Integer.parseInt(parts[2]);
        return year * 12 * 28 + month * 28 + day; // 모든 달이 28일로 고정
    }
    
    // 주어진 날짜에 몇 개월을 더한 날짜를 계산하는 메서드
    private int addMonthsToDate(int date, int months) {
        int days = date + months * 28; // 달은 28일로 계산
        return days;
    }
}
